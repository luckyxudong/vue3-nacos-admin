...
## Purpose
图标系统为 Vue3 模板项目提供完整的图标解决方案，支持多种图标使用方式：
- UnoCSS 图标：通过类名使用 Iconify 图标库中的图标
- 本地 SVG 图标：支持使用 UI 设计师提供的自定义 SVG 图标
- Icon 组件：提供统一的图标组件，支持 UnoCSS、Iconify、本地 SVG、Iconfont 四种图标类型
## Requirements
